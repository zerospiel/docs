...
* Finally, select one of the options:
    ...
    * [Istio](#istio) service mesh for secure connectivity between clusters.
      This is the only option which does not need the access to create external DNS records
...
and [Manual DNS config](./kof-verification.md#manual-dns-config) later,
apply the steps below to enable [Istio](https://istio.io/) service mesh:
...
3. Install the `kof-istio` chart to the management cluster:
    ...
    by passing values to [kof-istio-child](https://github.com/k0rdent/kof/blob/v{{{ extra.docsVersionInfo.kofVersions.kofDotVersion }}}/charts/kof-istio/templates/kof-child-cluster-profile.yaml)
...
2. Create the `mothership-values.yaml` file:
    ...
    to make it possible to reference them from the regional and child `MultiClusterServices`.
...
8. If you have not applied the [Istio](#istio) section:
    ...
    * If your regional clusters already have `ingress-nginx` and `cert-manager`,
        you can ask `kof-regional` chart to not install them by setting Helm values
        `ingress-nginx.enabled` and `cert-manager.enabled` to `false` below.
    * You may want to [customize collectors](https://github.com/k0rdent/kof/blob/v{{{ extra.docsVersionInfo.kofVersions.kofDotVersion }}}/docs/collectors.md#example)
...
4. If you've applied the [DNS auto-config](#dns-auto-config) section,
    add to the `.metadata.labels` in the `regional-cluster.yaml` file.
...
5. If you've applied the [Istio](#istio) section, update the `regional-cluster.yaml` file:
    ...
    * Replace this line:
      ...
      and `kof-` prefix is not missing in `istio-role`.
      This makes your config compatible with possible migration of Istio support
      from KOF to the upstream {{{ docsVersionInfo.k0rdentName }}},
