...
`ClusterTemplate` and `ProviderTemplate` are based on a Helm chart. So can a `ServiceTemplate`, but it can be also based on kustomization or raw resources. Regardless of the type of the resources to be deployed using template, you'll need to start by creating the corresponding source object.
